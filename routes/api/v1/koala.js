...
/* GET koala page. */
router.get("/api/v1/koala", limiter, function (req, res, next) {
  try {
    ...
    if (option == "both" || option == "image") {
      let bears = fs.readdirSync("./public/img/koala");
      image = bears[Math.floor(Math.random() * bears.length)];
      image = req.protocol + "://" + req.get("host") + "/img/koala/" + image;
    }

    let fact;
    if (option == "both" || option == "fact") {
      let facts = fs.readFileSync("./public/json/koalas-fact.json", "utf8");
